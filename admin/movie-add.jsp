...
<head>
    ...
    <script>
        function check_validation(){
           ...
           else if(document.getElementById("movie-img").value == ""){
              alert("영화 포스터 경로가 입력되지 않았습니다.");
              document.getElementById("movie-img").focus();
              return false;
           }
           else{ 		
			    alert('영화 추가에 성공하였습니다.');
                document.movieadd.action = "movie-addproc.jsp";
                document.movieadd.submit();
           }
        }
    </script>
</head>
    ...
    <div id="wrapper">
        <div id="navi-wrapper">
            <ul id="navi-bar">
                ...
                <li>
                    <center>
                        <a id="add" href="movie-add.jsp" style="background-color: white; color: #52c0dc;">영화 추가</a>
                    </center>
                </li>
                <li>
                    <center>
        ...
        <div class="content-zone">
            <form method="post" name="movieadd" id="form1">
                <div class="input-set">
        영화제목<input type="text" name="movie-title" id="movie-title" placeholder="추가할 영화 제목을 입력하세요 ">
  </div>
  <div class="input-set">
        상영시간<input type="time" name="movie-time" id="movie-time">
  </div>
  <div class="input-set">
        이미지 경로<input type="file" name="movie-img" id="movie-img">
  </div>
  <div class="input-set">
        <input type="button" id="submit-button" onclick="check_validation();" value="영화 추가">
  </div>
          </form>
        </div>           
      </div>
